...
export function IndividualMarket({ items }: any) {
  ...
  return doneFinding ? (<h1></h1>) :  
    <div className="IndividualMarketPage">
        <div className="MarketHeader">
            {marketObject.question}
        </div>
      <div className="MarketHeaderInfo">
        <a className="MarketHeaderText">
          <a>Volume</a>
          <a>{'$'+Math.round(marketObject.volume).toLocaleString("en-US")} </a>
        </a>
        <a className="MarketHeaderText">
          <a>Liquidity</a>
          <a>{'$'+Math.round(marketObject.liquidity).toLocaleString("en-US")}</a>
        </a>
        <a className="MarketHeaderText">
      ...
      <div className="flexContainsBuyAndGraph">
        <div className="BuyMenu">
            ...
            <div className="BuySellButton">
                <div className="ActualButton" onClick={() => {setBuyClicked(true)}} style={{borderColor: !buyClicked ? '' : 'green', color: !buyClicked ? '' : 'green' } }>
                    Buy
                </div>
                <div  className="ActualButton" onClick={() => {setBuyClicked(false)}} style={{borderColor: buyClicked ? '' : 'red', color: buyClicked ? '' : 'red' }} >
                    Sell
                </div>
            </div>

            <div className="BuySellButton">
                <div className="ActualButton"  onClick={() => {setYesClicked(true)}} style={{borderColor: !yesClicked ? '' : 'green', color: !yesClicked ? '' : 'green'  }}>
                    <a>{marketObject.outcomes[0]!}</a>
                    
                    <a>{"$"+Math.round(marketObject.outcomePrices[0]!*100)/100}</a>
                </div>
                <div className="ActualButton" onClick={() => {setYesClicked(false)}} style={{borderColor: yesClicked ? '' : 'red', color: yesClicked ? '' : 'red' }}>
                    <a>{marketObject.outcomes[1]!}</a>
                    <a>{"$"+Math.round(marketObject.outcomePrices[1]!*100)/100}</a>
                </div>
            </div>
            ...
            <div className="Amount">Amount
                <span style={{fontSize: '10px',color: 'grey'}}> Set order size</span>
            </div>
